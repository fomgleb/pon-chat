...
GlobalRoom::GlobalRoom(msock::tcp::TcpClient<true>& opened_tcp_client, ui::GlobalRoomUI& ui)
    : tcp_client_(opened_tcp_client), ui_(ui)
{
    ui_.SubscribeToEnteredUserNameEvent(std::bind(&GlobalRoom::OnEnteredUsername, this, std::placeholders::_1));
    ui_.SubscribeToEnteredMessageEvent(std::bind(&GlobalRoom::OnEnteredMessage, this, std::placeholders::_1));
}
...
void GlobalRoom::StartLoop()
{
    std::thread handle_messages_from_server_thread([&] {
        while (true) {
            std::optional<grp::Message> received_message = grp::ReceiveMessage(tcp_client_);
