...
class GlobalRoomUI {
  public:
    ...
    virtual void SubscribeToEnteredUserNameEvent(std::function<void(const std::string& username)> func) = 0;
    virtual void SubscribeToEnteredMessageEvent(std::function<void(const std::string& message)> func) = 0;
    ...
    virtual void AddAndDrawNewMessage(protocols::global_room_protocol::Message& message) = 0;
